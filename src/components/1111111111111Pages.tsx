...
export const HomeWifiPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <PageLayout title={title} icon={Wifi} onBack={onBack}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        <div>
          <h3 className="text-3xl font-bold mb-6 text-slate-900 dark:text-white">
            {getBlock('wifi_hero_title')}
          </h3>
          <p className="text-lg leading-relaxed mb-8">
            {getBlock('wifi_hero_text')}
          </p>

          
        </div>

        {/* === Переваги Mesh — точно як у тебе === */}
        <div>
          ...
          <ul className="space-y-4">
            {[1, 2, 3].map(i => (
              <li key={i} className="flex gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl">
                <div className="mt-1 text-brand-500">
                  <Wifi size={20} />
                </div>
                {/* Ось правильні id — тепер текст є! */}
                <div>
                  <div className="font-semibold text-slate-800 dark:text-gray-200">
                    {getBlock(`mesh_feat${i}_title`)}
                  </div>
                  <div className="text-sm text-slate-600 dark:text-gray-400 mt-1">
                    {getBlock(`mesh_feat${i}_text`)}
                  </div>
                </div>
              </li>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-[24px] border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row gap-6 items-center">
          <div className="w-full md:w-1/3 aspect-square bg-slate-100 dark:bg-white/5 rounded-2xl flex items-center justify-center p-4">
            <img
              src="https://static.tp-link.com/upload/product-overview/2023/202311/20231128/EX220%20(EU)%201.0_01_normal20231128020008.jpg"
              alt="TP-Link EX-220"
          ...
          <div className="flex-1">
            <h4 className="font-bold text-xl text-slate-900 dark:text-white mb-2">
              {getBlock('router1_name')}
            </h4>
            <p className="text-sm text-slate-600 dark:text-gray-400 mb-4">
              {getBlock('router1_desc')}
            </p>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-[24px] border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row gap-6 items-center">
          <div className="w-full md:w-1/3 aspect-square bg-slate-100 dark:bg-white/5 rounded-2xl flex items-center justify-center p-4">
            <img
              src="https://static.tp-link.com/upload/product-overview/2021/202108/20210810/EC220-G5(EU)1.jpg"
              alt="TP-Link EC-220"
              className="w-full h-auto object-contain mix-blend-multiply dark:mix-blend-normal"
            />
          </div>
          <div className="flex-1">
            <h4 className="font-bold text-xl text-slate-900 dark:text-white mb-2">
              {getBlock('router2_name')}
            </h4>
            <p className="text-sm text-slate-600 dark:text-gray-400 mb-4">
              {getBlock('router2_desc')}
            </p>
            <div className="flex gap-2">
      ...
      </div>{/* Цей блок у тебе був, але id не існував — тепер працює */}
          <div className="bg-brand-50 dark:bg-brand-900/10 p-6 rounded-2xl border border-brand-100 dark:border-brand-500/20">
            <p className="font-medium text-brand-800 dark:text-brand-400">
              {getBlock('service_text')} {/* ← тепер є цей блок у даних */}
            </p>
          </div>

      {/* === Професійні рішення — точно як у тебе === */}
